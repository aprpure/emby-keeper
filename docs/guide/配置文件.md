...
| `llm` | `dict` | `skip_auth` 开启后的本地替代配置子项 | |
...
| `skip_auth` | `bool` | 设为 `true` 后, 不再使用 `@embykeeper_auth_bot`, 改为使用本地 LLM / helper 替代 | `false` |
...
- `skip_auth = false`: 所有 `Link` 能力继续走远端 `@embykeeper_auth_bot`.
- `skip_auth = true`: 所有原本发给远端认证机器人的能力切换为本地替代, 优先使用 `llm` 配置, 部分能力可选用 `llm.helper_command` 承接.

更完整的能力映射和注意事项见 [docs/guide/LLM-本地替代.md](LLM-%E6%9C%AC%E5%9C%B0%E6%9B%BF%E4%BB%A3.md).
...
skip_auth = true # 改为本地 LLM / helper 替代远端认证机器人
...
该子项只在 `telegram.account.skip_auth = true` 时参与远端能力替代.

<!-- prettier-ignore -->
| 设置项 | 值类型 | 简介 | 默认值 |
| ----- | ----- | ---- | ------ |
| `mode` | `str` | 本地替代模式, 可选 `auto` / `local` / `remote` | `auto` |
| `api_key` | `str` | OpenAI 兼容接口 API Key | |
| `base_url` | `str` | OpenAI 兼容接口基础地址 | `https://api.openai.com/v1` |
| `model` | `str` | 文本模型名 | `gpt-4o-mini` |
| `vision_model` | `str` | 视觉模型名, 不填则复用 `model` | |
| `timeout` | `int` | LLM 请求超时 (秒) | `60` |
| `helper_command` | `str` | 本地 helper 命令, 用于承接 captcha / wssocks / 推送等能力 | |
| `helper_timeout` | `int` | helper 超时 (秒) | `120` |
| `auth_services` | `list[str]` | 手动声明由 helper 支持的服务名 | |
...
如果您还需要替代验证码令牌、Cloudflare 会话或 Telegram 日志推送, 还需要配置 `helper_command`. 详细说明见 [docs/guide/LLM-本地替代.md](LLM-%E6%9C%AC%E5%9C%B0%E6%9B%BF%E4%BB%A3.md).
